...
import { IEquipmentApiModel, IEquipmentCategoryApiModel, IEquipmentPriceApiModel } from '../../interfaces/api-models';
...
type Props = {
    handleSubmitEquipment: (equipment: IEquipmentApiModel) => void;
    equipment?: Equipment;
    formId: string;
};
...
const EquipmentForm: React.FC<Props> = ({ handleSubmitEquipment, equipment: equipment, formId }: Props) => {
    ...
    const equipmentCategoriesFetcher = (url: string) =>
        fetch(url)
            .then((apiResponse) => getResponseContentOrError<IEquipmentCategoryApiModel[]>(apiResponse))
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (form.prices?.value) {
            ...
            try {
                const prices = JSON.parse(form.prices.value) as IEquipmentPriceApiModel[];
                if (
                    prices.some(
                        (price) =>
        ...
        const modifiedEquipment: IEquipmentApiModel = {
            id: equipment?.id,
