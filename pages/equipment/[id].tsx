...
import { IEquipmentApiModel } from '../../interfaces/api-models';
...
const EquipmentPage: React.FC<Props> = ({ user: currentUser }: Props) => {
    ...
    const handleSubmit = async (equipment: IEquipmentApiModel) => {
        ...
        fetch('/api/equipment/' + router.query.id, request)
            .then((apiResponse) => getResponseContentOrError<IEquipmentApiModel>(apiResponse))
            .then(toEquipment)
            .then((equipment) => {
